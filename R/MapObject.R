MapObject <- R6::R6Class("MapObject",
  public = list(
    ...
    initialize = function(dataset1, target_col1, dataset2=NULL, target_col2=NULL, samples1=NULL,
        ...
        self$dataset1 <- dataset1 %>% arrange(.data[[target_col1]])
        if (discard_dups) {
            self$dataset1 <- self$dataset1 %>% distinct(.data[[target_col1]], .keep_all=TRUE)
        }
        self$target_col1 <- target_col1

        if (!is.null(dataset2) && !is.null(target_col2)) {
            self$dual_datasets <- TRUE
            self$dataset2 <- dataset2 %>% arrange(.data[[target_col2]])
            if (discard_dups) {
                self$dataset2 <- self$dataset2 %>% distinct(.data[[target_col2]], .keep_all=TRUE)
            }
        ...
        if (matched) {
            ...
            if (length(self$samples1) != length(self$samples2)) {
                ...
                ref_rdf_joint <- ref_rdf %>%
                    dplyr::filter(.data[[self$target_col1]] %in% joint_ids) %>%
                    arrange(.data[[self$target_col1]])
                comp_rdf_joint <- comp_rdf %>%
                    dplyr::filter(.data[[self$target_col2]] %in% joint_ids) %>%
                    arrange(.data[[self$target_col2]])
        ...
        corrs <- lapply(
            ...
            function(corr_type) {
                lapply(seq_len(nrow(ref_sdf_joint)), function(row_i, ref_mat, comp_mat, corr_type, ids) {
                    ...
                    do.call("rbind", .) %>%
                    rename_with(~paste(corr_type, ., sep="."))
            }
        ) %>% 
            do.call("cbind", .) %>% 
            dplyr::mutate(
    ...
    get_combined_dataset = function(only_no_na_entries = FALSE, include_one_dataset_entries = TRUE) {
        ...
        if (!is.null(self$dataset1) && !is.null(self$dataset2)) {
            ...
            if (!include_one_dataset_entries) {
                ...
                out_df1 <- self$dataset1 %>% rename_with(~paste0("d1.", .))
                out_df2 <- self$dataset2 %>% rename_with(~paste0("d2.", .))
        ...
        if (nrow(out_df) > 0) {
            cbind(comb_id = paste0("C", seq_len(nrow(out_df))), out_df)
        }
        else {
            NULL
        }
