MapObject <- R6Class("MapObject", list(
    ...
    get_full_entries = function(dataset) {
        dataset %>% filter(complete.cases(dataset) & !apply(dataset, 1, function(elem) { any(is.infinite(elem)) } ))
    },
    ...
    get_combined_dataset = function(full_entries = FALSE) {
        if (!is.null(self$dataset1) && !is.null(self$dataset2)) {
            ...
            if (full_entries) {
                out_df1 <- out_df1 %>% self$get_full_entries()
                out_df2 <- out_df2 %>% self$get_full_entries()
            }
            ...
            if (full_entries) {
                out_df <- out_df %>% self$get_full_entries()
            }
